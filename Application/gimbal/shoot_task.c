...
uint16_t friction_wheel_speed[3] = {5900, 6400, 7700}; //4900-16.7m/s  5900-20m/s    7000-24m/s      8000-25m/s//6955   //4480   
                                        //测试500转
//4420,4920,7150
...
uint16_t nowtime=0;
uint16_t error_angle=0;
uint16_t shoot_comps=3;
uint16_t fire_flag;

uint16_t two_interval_time;
uint16_t two_all_time;
uint16_t two_last_time;
uint16_t two_now_time;
/* 扩展变量 ------------------------------------------------------------------*/
...
void ShootTask(void const*argument)
{
    ...
    for(;;)
    {
        ...
        Shoot_LaserCtrl(shoot_handle.ctrl_mode);			
			//Shoot_HeatingTime(&shoot_handle);
		  //	Shoot_HeatingLimit(&shoot_handle);
        ...
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, shoot_handle.magazine_pwm);
...
void ShootTaskInit(void)
{
	...
	shoot_handle.vision_tx_data = RobotInfo_Pointer(); 
    shoot_handle.magazine_initial_degree = 20;
    shoot_handle.magazine_90_degree = 80;
...
static void ShootSensorUpdata(void)
{
    ...
    shoot_handle.friction_wheel_speed_buff = shoot_handle.console->friction_wheel_speed_buff;
    
  	shoot_handle.shoot_speed=(fabs(shoot_handle.fric_wheel_motor[0].motor_info->speed_rpm/60*FRIC_WHEEL_RADIUS/1000)
	                          +fabs(shoot_handle.fric_wheel_motor[1].motor_info->speed_rpm/60*FRIC_WHEEL_RADIUS/1000))/2;  //计算转速
	  
	
    if (!CheckDeviceIsOffline(OFFLINE_REFEREE_SYSTEM))
    {
       shoot_handle.shooter_heat_cooling_rate = RefereeSystem_RobotState_Pointer()->shooter_barrel_cooling_value;
       shoot_handle.shooter_heat_cooling_limit = RefereeSystem_RobotState_Pointer()->shooter_barrel_heat_limit;			
...
//			  shoot_handle.shooter_speed_limit = RefereeSystem_RobotState_Pointer()->shooter_id1_17mm_speed_limit;
       shoot_handle.shooter_heat = RefereeSystem_PowerHeatData_Pointer()->shooter_id1_17mm_cooling_heat;
			 shoot_handle.shooter_speedfeedback=RefereeSystem_SpeedData_Pointer()->initial_speed;
...
static void Shoot_TriggerMotorCtrl(ShootHandle_t* handle)
{
    ...
    if (handle->ctrl_mode == SHOOT_START)           //开始射击模式
    {
        ...
        {									
					if(handle->console->shoot.fire_cmd == TWO_FIRE_CMD)               //自动拨弹模式  //二连射
					{
						if(two_all_time>200||handle->console->shoot.fire_cmd!=handle->console->shoot.last_fire_cmd)  //0.2s间隔
						{
						    fire_flag=1;						
						}
						if(fire_flag==1)
            ...
            /*可以发弹量 = (最大热量 - 枪口热量) / 一发子弹热量 */
					...
					{
					  max_bullet_nums = ((handle->shooter_heat_cooling_limit - handle->shooter_heat) / ONE_BULLET_HEAT)-2;     //shooter_heat2为自算热量
          }
...
float test_speed=22; //三号23          //四号22
static void Shoot_FrictionWheelMotorCtrl(ShootCtrlMode_e mode, FrictionWheelMotor_t motor[2])   //摩擦轮射击模式
{
    ...
    if (mode == SHOOT_START)
    {
        for (uint8_t i = 0; i<3; i++)
        {
            ...
            {
                motor[0].set_speed = -(friction_wheel_speed[i] + shoot_handle.friction_wheel_speed_buff);
                motor[1].set_speed = friction_wheel_speed[i] + shoot_handle.friction_wheel_speed_buff;
...
//{     	
//	if (handle->console->shoot.fire_cmd == ONE_FIRE_CMD)
//	{                
//		fire_flag=1;
//	}
//	else
...
{	 
    if(handle->ctrl_mode == SHOOT_START||shoot_handle.shooter_heat!=0)     //开启射弹模式(枪口热量计算)
		{
			 shoot_handle.shooter_heat2 =shoot_handle.shooter_heat2+shoot_flag*10-shoot_handle.shooter_heat_cooling_rate; 			
			//枪口实际热量=上次枪口实际热量+实际射弹热量-冷却
			if(shoot_handle.shooter_heat2<30)
			{
			   shoot_handle.shooter_heat2=39;            //热量闭环预支
			}
		}
    if(shoot_handle.shooter_heat2-shoot_handle.shooter_heat>4*shoot_handle.shooter_heat_cooling_rate_k)  //弹仓无弹判定（枪口热量计算）
			//(4为枪口热量系数可自行修改按情况)			
		{
		    shoot_handle.shooter_heat = RefereeSystem_PowerHeatData_Pointer()->shooter_id1_17mm_cooling_heat;	
			  shoot_handle.shooter_heat2 = shoot_handle.shooter_heat+29;
		}
}
